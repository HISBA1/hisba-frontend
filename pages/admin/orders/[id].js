...
const OrderDetail = () => {
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            ...
            try {
                const response = await axios.get(`https://hisba-backend.onrender.com/api/orders/${id}/`, {
                    headers: {
    ...
    const handleApproveOrder = async () => {
        try {
            await axios.post(`https://hisba-backend.onrender.com/api/orders/approve/${id}/`, {}, {
                headers: {
                    Authorization: `Bearer ${session?.user?.accessToken}`
                }
            });
                                ...
                                <Grid item xs={12} sm={6} md={4} key={item.id}>
                                    <Card className={styles['item-card']}>
                                        <CardContent>
                                            ...
                                            <Typography variant="body2">Total: {item.total_price} ILS</Typography>
                                        </CardContent>
                                    </Card>
                                </Grid>
                            ))}
